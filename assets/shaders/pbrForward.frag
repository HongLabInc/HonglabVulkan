...
vec3 getIBLRadianceGGX(vec3 n, vec3 v, float roughness)
{
    ...
    // v is surface→camera (outgoing). GLSL reflect() expects an incident
    // vector (camera→surface), so negate. The result is the surface→environment
    // direction used to sample the prefiltered environment cubemap.
